// /api/blue-marlin-stats.js
// Pulls Google Search Console data for Blue Marlin Pool Care and returns
// portal-friendly JSON. Caches in-memory for 24h per Vercel instance.
//
// Required env vars (set in Vercel project settings):
//   GSC_OAUTH_CLIENT_ID               — OAuth 2.0 client ID (shared)
//   GSC_OAUTH_CLIENT_SECRET           — OAuth 2.0 client secret (shared)
//   GSC_OAUTH_REFRESH_TOKEN_BLUEMARLIN — refresh token for Blue Marlin GSC property
//   GSC_BLUEMARLIN_SITE_URL           — e.g. "sc-domain:bluemarlinpoolcare.com"
//                                         OR "https://bluemarlinpoolcare.com/"
//
...
function demoPayload(siteUrl) {
  ...
  return {
    site: 'blue-marlin',
    domain: 'bluemarlinpoolcare.com',
    last_refreshed: new Date().toISOString(),
    from_cache: false,
    is_demo: true,
    demo_reason: 'GSC OAuth env vars not configured yet — add GSC_OAUTH_REFRESH_TOKEN_BLUEMARLIN once domain is verified in Google Search Console',
    summary: {
      clicks: 0,
      impressions: 0,
      ctr: 0,
    ...
    top_pages: [
      ...
      { path: '/services.html', clicks: 0, impressions: 0, pct: 0 },
      { path: '/pool-report.html', clicks: 0, impressions: 0, pct: 0 },
      { path: '/maintenance.html', clicks: 0, impressions: 0, pct: 0 },
      { path: '/why-pool-care.html', clicks: 0, impressions: 0, pct: 0 },
    ],
  };
}
...
async function fetchLive(siteUrl) {
  ...
  const refreshToken = process.env.GSC_OAUTH_REFRESH_TOKEN_BLUEMARLIN;
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error('Missing GSC OAuth env vars (GSC_OAUTH_CLIENT_ID / GSC_OAUTH_CLIENT_SECRET / GSC_OAUTH_REFRESH_TOKEN_BLUEMARLIN)');
  }
...
module.exports = async function handler(req, res) {
  ...
  const siteUrl = process.env.GSC_BLUEMARLIN_SITE_URL || 'sc-domain:bluemarlinpoolcare.com';
  const refresh = req.query?.refresh === 'true' || req.query?.refresh === '1';

  // Cache check
  const cacheKey = `blue-marlin:${siteUrl}`;
  ...
  if (!process.env.GSC_OAUTH_REFRESH_TOKEN_BLUEMARLIN) {
  ...
  try {
    ...
    console.error('blue-marlin-stats error:', err);
